...
function isOperationalLicense(status: string, expiresAt: Date | null) {
...
export async function portalRoutes(app: FastifyInstance) {
  app.post('/portal/auth/register', async (request) => {
    ...
    const [client, portalUser] = await prisma.$transaction(async (tx) => {
      ...
      return [updatedClient, createdPortalUser] as const
    })
      ...
      ...(query.search
        ? {
            OR: [
              ...
              { client: { tradeName: { contains: query.search, mode: 'insensitive' as const } } }
            ]
          }
        : {})
    }
  ...
  app.post('/portal/users', { preHandler: requireAdminAuth }, async (request) => {
    ...
    if (existing) throw new AppError('Ja existe um acesso com este e-mail.', 409)
  ...
  app.post('/portal/users/:id/reset-password', { preHandler: requireAdminAuth }, async (request) => {
    ...
    if (!existing) throw new AppError('Conta do cliente nao encontrada.', 404)
