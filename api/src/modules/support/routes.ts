...
function mapSupportTicket(item: any) {
  return {
    id: item.id,
...
export async function supportRoutes(app: FastifyInstance) {
      ...
      ...(query.search
        ? {
            OR: [
              ...
              { portalUser: { email: { contains: query.search, mode: 'insensitive' as const } } }
            ]
          }
        : {})
    }
  ...
  app.get('/support/tickets/:id', { preHandler: requireAdminAuth }, async (request) => {
    ...
    if (!item) throw new AppError('Ticket nao encontrado.', 404)
  ...
  app.post('/support/tickets/:id/reply', { preHandler: requireAdminAuth }, async (request) => {
    ...
    if (!existing) throw new AppError('Ticket nao encontrado.', 404)
  ...
  app.post('/portal/tickets/:id/messages', { preHandler: requirePortalAuth }, async (request) => {
    ...
    if (!ticket) throw new AppError('Ticket nao encontrado.', 404)
