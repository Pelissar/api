...
import { licenseActivationSchema, licenseSchema, licenseValidationSchema, paginationQuerySchema, renewLicenseSchema, updateCheckSchema } from '@nexus/shared';
...
function isOperationalLicense(status) {
...
function allowVersionByLicenseBounds(current, version) {
    if (current.minVersion && compareVersions(version, current.minVersion) < 0)
        return false;
    if (current.maxVersion && compareVersions(version, current.maxVersion) > 0)
        return false;
    return true;
}
export async function licenseRoutes(app) {
            ...
            ...(query.search
                ? {
                    OR: [
                        ...
                        { client: { cpfCnpj: { contains: query.search, mode: 'insensitive' } } }
                    ]
                }
                : {})
        };
    ...
    app.post('/licenses/check-update', async (request) => {
        ...
        const candidates = versions.filter((version) => {
            if (!allowVersionByLicenseBounds(license, version.version))
                return false;
            if (version.allowAllActiveLicenses)
                return true;
