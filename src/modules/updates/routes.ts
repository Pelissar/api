...
function parseBooleanValue(input, fallback = false) {
    if (input == null)
        return fallback;
...
export async function updateRoutes(app) {
    ...
    app.get('/updates/latest', async (request) => {
        ...
        const tenantId = request.auth?.tenantId || request.query.tenantId;
        if (!tenantId) {
    ...
    app.post('/updates', { preHandler: requireAdminAuth }, async (request) => {
        ...
        for await (const part of parts) {
            ...
            if (part.fieldname === 'releaseChannel')
                releaseChannel = String(part.value || 'STABLE');
            if (part.fieldname === 'isMandatory')
                isMandatory = parseBooleanValue(String(part.value || 'false'));
            if (part.fieldname === 'isActive')
                isActive = parseBooleanValue(String(part.value || 'true'), true);
            if (part.fieldname === 'allowAllActiveLicenses')
                allowAllActiveLicenses = parseBooleanValue(String(part.value || 'true'), true);
            if (part.fieldname === 'targetPlanIds')
                targetPlanIds = safeJsonParse(String(part.value || '[]'), []);
            if (part.fieldname === 'targetLicenseIds')
                targetLicenseIds = safeJsonParse(String(part.value || '[]'), []);
        }
        if (!storedFile)
    ...
    app.get('/downloads/:versionId', async (request, reply) => {
        ...
        const isAllowed = version.allowAllActiveLicenses ||
