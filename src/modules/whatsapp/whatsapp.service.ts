...
import type { WhatsAppConversationSummary, WhatsAppMessageSummary, WhatsAppSettingsSummary, WhatsAppSuggestionSummary } from './whatsapp.types.js'
import {
  createOrUpdateInboundConversation,
  createSuggestion,
  ...
  listAllWhatsAppSettings,
  listConversationsByTenant,
  listSuggestionsByConversation,
  saveOutboundMessage,
...
function mapMessage(item: any): WhatsAppMessageSummary {
  return {
    id: item.id,
...
function mapSuggestion(item: any): WhatsAppSuggestionSummary {
  return {
    id: item.id,
    conversationId: item.conversationId,
...
function mapConversation(item: any): WhatsAppConversationSummary {
  return {
    id: item.id,
...
export class WhatsAppService {
  constructor(
    private readonly metaService = new WhatsAppMetaService(),
    private readonly aiService = new WhatsAppAiService()
  ) {}

  async getSettings(tenantId: string): Promise<WhatsAppSettingsSummary> {
  ...
  ): Promise<WhatsAppSettingsSummary> {
  ...
  async resolveSettingsForTenant(tenantId: string) {
    const saved = await findWhatsAppSettingsByTenant(tenantId)
  ...
  async listConversations(tenantId: string, options?: { search?: string; status?: string }) {
  ...
  async generateSuggestion(tenantId: string, conversationId: string, explicitSourceMessageId?: string | null) {
  ...
  async sendMessage(tenantId: string, conversationId: string, messageText: string, suggestionId?: string | null) {
    const conversation = await getConversationById(tenantId, conversationId)
    if (!conversation) {
      throw new AppError('Conversa nao encontrada.', 404)
    }
  ...
  async setConversationStatus(tenantId: string, conversationId: string, status: 'OPEN' | 'WAITING_USER' | 'CLOSED') {
  ...
  async setSuggestionStatus(tenantId: string, conversationId: string, suggestionId: string, status: 'PENDING' | 'APPROVED' | 'EDITED' | 'REJECTED' | 'SENT') {
    const conversation = await getConversationById(tenantId, conversationId)
    if (!conversation) {
      throw new AppError('Conversa nao encontrada.', 404)
    }
